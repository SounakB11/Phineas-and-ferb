    ...
    children=[
        html.Div(
            children=[
                ...
                html.P(
                    children="Input the username of the account"
        ...
        html.Div(
            children=[
                html.Div(
                    children=[
                        html.Div(children="Username", className="menu-title"),
                        html.Div(
                            dcc.Input(id="username", type="text",
                            style={'height': '42px', 'width': '200px','font-size': '15px'}),
                            className="dropdown",
                        ),
...
def cond(n_clicks, username):


    # Loading a profile from an Instagram handle
    bot = instaloader.Instaloader(title_pattern='pfp')
    profile = instaloader.Profile.from_username(bot.context, username)
    bot.download_profilepic(profile)
    pfp_path = username + '/pfp.jpg'
    bio = profile.biography
    full_name = profile.full_name

    #Read downloaded profile picture and compare to blank profile picture
    img1 = cv2.imread('Blank_PFP.jpg')
    img2 = cv2.imread(pfp_path)
    ...
    pfpexists = (mse>1)
    nlu = sum(c.isdigit() for c in username)/len(username)
    wfn = len(full_name.split())
    if (len(full_name) == 0):
        nlfn = 0
    else:
        nlfn = sum(c.isdigit() for c in full_name)/len(full_name)
    fnu = (full_name == username)
    dl = len(bio)
    url = (profile.external_url == 1)
    po = profile.mediacount
    flwer = profile.followers
    flw = profile.followees
    private = profile.is_private


    if not any(pd.isnull(np.array([pfpexists,nlu,wfn,nlfn,fnu,dl,url,po,flwer,flw,private]))):
        if (clf.predict(np.array([[pfpexists,nlu,wfn,nlfn,fnu,dl,url,po,flwer,flw,private]]))==1):
